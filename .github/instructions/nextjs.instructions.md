...
- **Top-level folders & files:**
  ...
  - `proxy.ts` — Network boundary request/response modification (replaces legacy `middleware.ts`)
...
## 3. Next.js 16+ Async Request APIs (App Router)

- **Request-bound data is strictly async.** In Next.js 16, APIs like `cookies()`, `headers()`, and `draftMode()` are asynchronous and **must** be `await`ed. Synchronous access has been fully removed.
- **Route props are Promises:** `params` and `searchParams` are strictly Promises. You **must** `await` them in Server Components, Route Handlers, and metadata functions (e.g., `generateMetadata`, `generateViewport`). In synchronous Client Components, unwrap them using `React.use()`.
...
- **Use the Node.js runtime (default):** Avoid the Edge runtime. Next.js 16 phases out the Edge runtime for user-space routing (it is unsupported for Cache Components and Proxy).
...
- **Authentication:** Protect sensitive routes using **Proxy (`proxy.ts`)** or server-side session checks. 
...
- **ESLint & Prettier:** Enforce code style and linting. Use the official Next.js ESLint config. Note that **`next lint` has been completely removed in Next.js 16**. You must run ESLint directly via the ESLint CLI (e.g., `eslint .`).
...
## 8. Caching & Revalidation (Next.js 16 Cache Components)
...
- **Proxy replaces Middleware:** The `middleware.ts` file convention is completely deprecated. Use `proxy.ts` (exporting a `proxy` function). It now runs strictly on the **Node.js runtime**, not Edge.
