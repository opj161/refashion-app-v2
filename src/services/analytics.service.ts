...
export function getDashboardKpis(): Omit<KpiData, 'totalStorageUsed'> {
  ...
  // using conditional aggregation. This prevents multiple passes over the recent history rows.
  // Note: COALESCE is required because SQLite's SUM() returns NULL on an empty dataset.
