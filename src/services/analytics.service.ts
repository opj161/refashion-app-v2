...
export function getDashboardKpis(): Omit<KpiData, 'totalStorageUsed'> {
  ...
  // with conditional aggregation to avoid scanning the idx_history_timestamp index 3 times.
  ...
  const result = stmt.get(twentyFourHoursAgo) as { generations24h: number; failedJobs24h: number; activeUsers24h: number; };
