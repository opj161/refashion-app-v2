...
import { Buffer } from 'buffer'; // Needed if we use fal.storage.upload with Buffers
...
export async function startVideoGenerationAndCreateHistory(input: GenerateVideoInput) {
  ...
  // 3. Submit the job to fal.ai using the webhook-specific approach
  // According to Fal.ai docs, we need to use the direct API with fal_webhook query parameter
  try {
    // Start with minimal required parameters
    const falInput: any = {
      prompt: input.prompt,
    ...
    console.log('Fal.ai input parameters for webhook submission:', JSON.stringify(falInput, null, 2));
    console.log('Webhook URL:', webhookUrl);
    
    // Use direct HTTP request to queue.fal.run with fal_webhook parameter
    const webhookResponse = await fetch(`https://queue.fal.run/fal-ai/bytedance/seedance/v1/lite/image-to-video?fal_webhook=${encodeURIComponent(webhookUrl)}`, {
      method: 'POST',
      headers: {
    ...
    console.log('Webhook submission successful:', webhookResult);
    const request_id = webhookResult.request_id || webhookResult.gateway_request_id;
    ...
    await updateVideoHistoryItem({ 
      ...
      seedUsed: null,
      status: 'processing',
      // Store taskId in the params for potential client-side polling
    });

    return { taskId: request_id, historyItemId: historyItemId };
  } catch (error: any) {
