...
async function comprehensiveTest() {
  ...
  try {
    ...
    const totalHistoryCount = db.prepare('SELECT COUNT(*) as count FROM history').get() as { count: number };
    const totalImageCount = db.prepare('SELECT COUNT(*) as count FROM history_images').get() as { count: number };
    ...
    for (const stat of userStats) {
      console.log(`      ${(stat as any).username}: ${(stat as any).total_items} total (${(stat as any).video_items} videos, ${(stat as any).image_items} images)`);
    }
    ...
    const testTransaction = db.transaction(() => {
      ...
      // Verify insertion
      const inserted = db.prepare('SELECT COUNT(*) as count FROM history WHERE id = ?').get(testId) as { count: number };
      if (inserted.count !== 1) {
    ...
    for (const step of plan) {
      if ((step as any).detail && (step as any).detail.includes('INDEX')) {
        usingIndex = true;
        break;
      }
    }
    ...
    const historyActions = await import('../src/actions/historyActions');
