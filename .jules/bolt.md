...
## 2024-05-24 - [Conditional Aggregation COUNT() Returns NULL]
**Learning:** When optimizing SQLite database performance by consolidating multiple `COUNT(*)` queries into a single query using conditional aggregation (e.g., `SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END)`), SQLite's `SUM()` function returns `NULL` on an empty dataset (no rows matching the `WHERE` clause). This differs from `COUNT()` which correctly returns `0`.
**Action:** Always wrap `SUM()` based conditional aggregations in `COALESCE(SUM(...), 0)` to guarantee a number is returned and avoid application crashes from unexpected nulls in numeric fields.
